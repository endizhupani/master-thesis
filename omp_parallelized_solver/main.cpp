...
#pragma omp parallel default(none) private(i) shared(u,w) reduction(+:mean)
    {
...
#pragma omp parallel default(none) private(i) firstprivate(u,w, num_iter) //reduction(max:globalDiff)
        {
            #pragma omp for schedule(static, getChunkSize(N*N))
